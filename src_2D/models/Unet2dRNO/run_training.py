...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train the 2D sinogram completion model with RNO.")
    ...
    parser.add_argument("--checkpoint-dir", type=Path, default=PROJECT_ROOT / "outputs/2d/checkpoints_rno")
    parser.add_argument("--figures-dir", type=Path, default=PROJECT_ROOT / "outputs/2d/visualisation_rno")
    parser.add_argument("--use-wandb", action="store_true")
    parser.add_argument("--wandb-project", type=str, default="dbt-sinogram-completion-2D-RNO")
...
def main() -> None:
    ...
    model = RadonInformedUNet(geometry_config=geometry, in_channels=1, out_channels=1, filters=args.filters).to(device)

    if torch.cuda.device_count() > 1:
        print(f"Utilisation de {torch.cuda.device_count()} GPUs avec nn.DataParallel !")
    ...
    try:
        for epoch in epoch_iter:
            ...
            for incomplete, target, _ in batch_iter:
                ...
                batch_iter.set_postfix(rno=f"{loss.item():.4f}")
    ...
    print(f"Training finished in {elapsed / 60:.1f} min. Best loss (RNO): {best_loss:.4f}")
    ...
    if (args.checkpoint_dir / "best_model.pt").exists():
        ...
        model_to_load.load_state_dict(torch.load(args.checkpoint_dir / "best_model.pt", map_location=device, weights_only=True))
